...
class NModel(nn.Module):
    def __init__(self,crop_len,embedding_num,class_num):
        ...
        self.hidd = nn.Linear(embedding_num,200)
        self.classifier = nn.Linear(200,class_num)
        self.tanh = nn.Tanh()
    ...
    def forward(self,x,label=None):
        ...
        if label is not None:
            loss = self.loss_fn(x.reshape(-1,x.shape[2]),y.reshape(-1))
            return loss
        return torch.argmax(x,dim=-1).reshape(-1)
