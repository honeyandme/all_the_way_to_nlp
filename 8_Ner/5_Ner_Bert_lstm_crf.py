...
class BLmodel(nn.Module):
    def __init__(self,hidden_num,class_num,bi):
        super().__init__()
        ...
        #mps用nn.lstm不知道为啥会出现未知错误，cuda或者cpu没问题
        self.lstm = nn.RNN(768,hidden_num,num_layers=1,batch_first=True,bidirectional=bi)
        if bi:
            self.classifier = nn.Linear(hidden_num*2,class_num)
        else:
            self.classifier = nn.Linear(hidden_num,class_num)

        self.crf = CRF(len(idx2tag),batch_first=True)
    def forward(self,x,label=None):
        bert_out_0,bert_out_1 = self.bert(x,attention_mask=(x>0),return_dict=False)
        lstm_out,_ = self.lstm(bert_out_0)
        pre = self.classifier(lstm_out)
        if label is not None:
            loss = -self.crf(pre,label)
            return loss
        else:
...
if __name__ == "__main__":
    ...
    model = BLmodel(hidden_num=hidden_num,class_num=len(tag2idx),bi=bi).to(device)
    if os.path.exists(f'best_{mode}_bert_lstm_crf.pt'):
        model.load_state_dict(torch.load(f'best_{mode}_bert_lstm_crf.pt'))
    ...
    for e in range(epoch):
        ...
        if f1>best_f1:
            ...
            # torch.save(model.state_dict(), f'best_{mode}_bert_lstm_crf.pt')
        else:
