...
import pickle
def get_data(path):
    s_path = os.path.join('data','pickle_fen_data_v1.t')
...
def build_word2idx(fen_text):
    mp = {"UNK": 0}
    path = os.path.join('data','pickle_word2idx_v1.t')
...
class Text_dataset(Dataset):
    def __init__(self,fen_text,grid_num):
    ...
    def __getitem__(self, idx):
        ...
        for word in sentence:
            mp[word] = idx2onehot(word2idx.get(word, 0), word2idx_len)
...
if __name__ == '__main__':
    ...
    dataset = Text_dataset(fen_text,grid_num)
