...
if __name__ == "__main__":
    ...
    for e in range(epoch):
        ...
        for bi,data in enumerate(tqdm(train_dataloader)):
            ...
            loss,sim = model(input_ids_a,att_a,input_ids_b,att_b,label)
            loss.backward()
            if bi%5==0:
                opt.step()
                opt.zero_grad()
            train_model_pre.extend((sim>0.5).long().detach().cpu().numpy())
        ...
        for bi,data in enumerate(tqdm(dev_dataloader)):
            ...
            sim = model(input_ids_a,att_a,input_ids_b,att_b)
            dev_model_pre.extend((sim>0.5).long().detach().cpu().numpy())
