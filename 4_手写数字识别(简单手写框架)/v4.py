...
if __name__ =='__main__':
    ...
    epoch = 100
    lr = 0.01
    linear1_layer = Linear(784,100)
    linear2_layer = Linear(100,10)
    sigmoid1_layer = Sigmoid()
    ...
    for e in range(epoch):
        ...
        for images,labels in dataset:
            #forward
            H = linear1_layer.forward(images)
            tanh_H = sigmoid1_layer.forward(H)
            pre = linear2_layer.forward(tanh_H)
            ...
            delta_tanhH = linear2_layer.backward(G)
            G2 = sigmoid1_layer.backward(delta_tanhH)
            linear1_layer.backward(G2)
        ...
        for X,Y in test_dataset:
            ...
            H = sigmoid1_layer.forward(H)
            pre = linear2_layer.forward(H)
            soft_pre = softmax(pre)
